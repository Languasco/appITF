...
@Component({
  selector: 'app-target-boticas-farmacias',
  templateUrl: './target-boticas-farmacias.component.html',
  styleUrls: ['./target-boticas-farmacias.component.css']
})
export class TargetBoticasFarmaciasComponent implements OnInit {
 ...
 getCargarCombos(){ 
  this.spinner.show();
  combineLatest([ this.actividadService.get_usuarios(this.idUserGlobal)  , this.categoriaService.get_categorias(), this.especialidadService.get_especialidades() ])
  .subscribe( ([ _usuarios,  _categorias, _especialidades ])=>{
 ...
 mostrarInformacion(){  
  ...
  this.spinner.show();
  this.targetService.get_mostrar_target_boticasFarmacias( this.formParamsFiltro.value)
...
//-----  CARGA MASIVA ASIGNACION DE USUARIOS  -------
 ...
 open_modalImportacion(opcionTarget:any){ 
  ...
  this.tituloTarget =  (opcionTarget =='A')? 'ALTA MASIVA DE TARGET DE BOTICAS Y FARMACIAS' : 'BAJA MASIVA DE TARGET DE BPOTICAS Y FARMACIAS';
  ...
  subirArchivo(){ 
    ...
    Swal.showLoading();
   this.uploadService.upload_Excel_target_boticasFarmacias( this.filesExcel[0].file , this.opcionTarget,   this.idUserGlobal ).subscribe(
     (res:RespuestaServer) =>{
      Swal.close();
       if (res.ok==true) { 
  ...
  guardar_importacion_target(){
    ...
    .then((result)=>{
      if(result.value){
        ...
        this.spinner.show();
        this.uploadService.save_archivoExcel_target_boticasFarmacias( this.opcionTarget, this.idUserGlobal )
        .subscribe((res:RespuestaServer) =>{  
            this.spinner.hide();   
            if (res.ok==true) { 
