...
@Component({
  selector: 'app-solicitud-boticas-farmacias',
  templateUrl: './solicitud-boticas-farmacias.component.html',
  styleUrls: ['./solicitud-boticas-farmacias.component.css']
})
 

export class SolicitudBoticasFarmaciasComponent implements OnInit {
 ...
 mostrarInformacion_solicitud(){ 
  ...
  this.spinner.show();  
      ...
      this.spinner.show();
      this.solicitudMedicoService.get_mostrar_solicitudBoticasFarmacias(this.formParamsFiltro.value, fechaIni,fechaFin )
 ...
 detalleSolicitudBoticasFarmacias(){ 
  this.solicitudMedicoService.get_solicitudMedicoDet(this.id_SolMedicoCab_Global)
      .subscribe((res:RespuestaServer)=>{  
          if (res.ok==true) {        
            this.editar(res.data[0]);
          }else{
            this.alertasService.Swal_alert('error', JSON.stringify(res.data));
            alert(JSON.stringify(res.data));
          }
